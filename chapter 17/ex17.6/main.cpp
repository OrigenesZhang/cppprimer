...
struct matches {
  ...
  matches(std::vector<Sales_data>::size_type st,
          std::vector<Sales_data>::const_iterator f,
          std::vector<Sales_data>::const_iterator s) : index(st), first(f), second(s) {}
};
  ...
  while (is >> s) {
    ...
    for (const auto &store : trans)
      os << "store " << store.index << " sales: "
         << std::accumulate(store.first, store.second, Sales_data(s))
         << std::endl;
  }
}
